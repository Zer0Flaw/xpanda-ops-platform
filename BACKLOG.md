...
- [ ] **Invoice Analytics — unit E: history tab.** A dedicated view over the full stored `freight_invoice_lines` dataset (reuses `GET /v2/api/logistics/flags`'s `perZip` summary) rather than only ever seeing flags in the context of one just-uploaded invoice.
